	...
	public void ClientAccountId (String TCID,String dbURL,String dbusername,String dbpassword,String Execute,String Results) throws Exception {
		...
		if (Execute.equals("Y")) {	
			try{			
				...
				String sqlQuery = "select clientAccountID from invdb.ext_acct_info where acctnum=2257;";
				String[] clientacctid = DBConnection.selectDataFromDB(dbURL,dbusername,dbpassword,sqlQuery);
				System.out.println(clientacctid);
				if (clientacctid.length != 0) 
				{
					String clientacctno = clientacctid[0];
					System.out.println(clientacctno);
					driver.close();
