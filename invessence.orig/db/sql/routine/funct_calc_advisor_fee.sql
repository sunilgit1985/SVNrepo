DROP FUNCTION `funct_calc_advisor_fee`;

DELIMITER $$
CREATE FUNCTION `funct_calc_advisor_fee`(
...
BEGIN
		...
		SELECT
			...
			`fee`.`adv_annual_basis`,
			case when (`fee`.`adv_annual_fixed` > 0)
					then `fee`.`adv_annual_fixed` - 0.009
				else 0.0
			end as annual_fixed,
			`fee`.`special_promotioninDays`,
			`fee`.`adv_promo_basis`,
			case when (`fee`.`adv_promo_fixed` > 0)
					then `fee`.`adv_promo_fixed` - 0.009
				else 0.0
			end as promo_fixed
		FROM `fee_structure` as `fee`
		WHERE `fee`.`advisor` = IFNULL(p_advisor,'Invessence')
		AND  `fee`.`min_capital` <= p_capital
		AND  `fee`.fixed_fee not in ('Y')
		-- AND  `fee`.`min_capital` <= p_capital
		;
	...
	BEGIN
		...
		WHERE `fee`.`advisor` = IFNULL(p_advisor,'Invessence')
		AND  `fee`.`min_capital` <= p_capital
		AND  `fee`.`max_capital` >= p_capital
		LIMIT 1;

		IF (t_useFixRate is null) 
			then set t_useFixRate = false;
		END IF;
		...
		IF (t_useFixRate)
			then
				...
				then
					set t_rate = (t_promo_rate/t_daysopened);
				else
					set t_rate = t_fix/t_daysinyear;
				end if;
