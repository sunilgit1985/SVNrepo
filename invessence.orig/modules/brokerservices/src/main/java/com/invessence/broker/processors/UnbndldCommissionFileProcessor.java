...
public class UnbndldCommissionFileProcessor
{
   private Logger logger = Logger.getLogger(UnbndldCommissionFileProcessor.class.getName());
   ...
   public List<Map<String, String>> process()
   {
      ...
      try
      {
         clearUnbldldCommissionTable();
         fileToProcess = FileProcessorUtil.getFileToProcess("UnbndldCommission");
   ...
   private void clearUnbldldCommissionTable()
   {
      JdbcTemplate clearTable = new JdbcTemplate(dataSource);

      String sql = "delete from tmp_unbundld_commission";
      ...
      if (file != null)
      {
         ...
         try
         {
            ...
            for (String line : allLines)
            {
               ...
               for (int i = 0; i < BrokerServiceConstants.UNBNDLD_COMMISSION_TABLE_COLUMN.length; i++)
               {
                  map.put(BrokerServiceConstants.UNBNDLD_COMMISSION_TABLE_COLUMN[i], strings[i]);
               }
