...
BEGIN
	...
	and `user_access_role`.`acctnum` = `user_trade_profile`.`acctnum`
	and `user_trade_profile`.`acctnum` = p_acctnum
	and upper(`user_access_role`.`role`) in ('OWNER')
	and `user_access_role`.`privileges` in ('F')
	;
END$$
DELIMITER ;
