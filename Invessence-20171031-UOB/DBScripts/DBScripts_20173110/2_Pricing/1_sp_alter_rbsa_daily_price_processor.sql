drop procedure if exists rbsa.daily_price_processor;
delimiter $$
CREATE PROCEDURE rbsa.daily_price_processor(in p_businessDate varchar(20),out dailymsg varchar(20),in p_dest_currency varchar(20))
 BEGIN
      ...
      volume,
      monthly_return,
    prev_month_businessdate,prev_monthly_adjusted,converted_adjusted_price,dest_currency FROM  rbsa.tmp_rbsa_daily;
    ...
    WHERE
        ticker_source_name = v_ticker and dest_currency=v_dest_currency;
      ...
      THEN
          ...
          AND dest_currency=v_dest_currency;
    ...
    WHERE
        ticker = v_ticker
            AND businessdate = v_prev_businessdate AND dest_currency=v_dest_currency;
    ...
    WHERE
        ticker = v_ticker
            AND businessdate = v_businessdate AND dest_currency=v_dest_currency;
      	if(r_count>0)
             then
           begin
             update rbsa_daily
            ...
            INSERT INTO rbsa_daily
      		 	  (ticker,businessdate,open_price,close_price,high_price,low_price,adjusted_price,volume,prev_businessdate,prev_close_price,daily_return,prev_month_businessdate,prev_monthly_adjusted,monthly_return,dest_currency,converted_adjusted_price,converted_prev_monthly_adjusted,exchangeRate,base_currency)
      	select ticker,businessdate,open_price,close_price,high_price,low_price,adjusted_price,volume,prev_businessdate,prev_close_price,daily_return,prev_month_businessdate,prev_monthly_adjusted,monthly_return,dest_currency,converted_adjusted_price,converted_prev_monthly_adjusted,ifnull(exchangeRate,1),ifnull(base_currency,dest_currency)
      	from tmp_rbsa_daily;
