drop procedure if exists rbsa.monthly_price_processor;
delimiter $$
CREATE PROCEDURE rbsa.monthly_price_processor(in  p_businessDate varchar(20), in  p_ticker varchar(20),in p_dest_currency varchar(20))
 BEGIN
        ...
        volume,monthly_return,converted_adjusted_price,dest_currency FROM  tmp_rbsa_daily where ticker =p_ticker ;
        ...
        v_volume,v_monthly_return,v_converted_adjusted_price,v_dest_currency;
        ...
        delete from rbsa_daily where ticker= p_ticker and dest_currency=p_dest_currency;
        		...
        		INSERT INTO rbsa_daily
        			  (ticker, businessdate, open_price, close_price, high_price, low_price, adjusted_price, prev_businessdate, prev_close_price, daily_return, volume, monthly_return,prev_month_businessdate,prev_monthly_adjusted,dest_currency,converted_adjusted_price,converted_prev_adjusted,converted_prev_monthly_adjusted,exchangeRate,base_currency)
        		select ticker, businessdate, open_price, close_price, high_price, low_price, adjusted_price, prev_businessdate, prev_close_price, daily_return, volume, monthly_return,prev_month_businessdate,prev_monthly_adjusted,dest_currency,converted_adjusted_price,converted_prev_adjusted,converted_prev_monthly_adjusted,ifnull(exchangeRate,1),ifnull(base_currency,dest_currency)
        		from tmp_rbsa_daily
        		where ticker=p_ticker;
