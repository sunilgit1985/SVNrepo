DROP TABLE IF EXISTS `testing`.`cleanup_acct` ;

CREATE TABLE `testing`.`cleanup_acct` (
...
DROP PROCEDURE IF EXISTS `testing`.`cleandb_of_testing_data`;

DELIMITER $$
CREATE PROCEDURE `testing`.`cleandb_of_testing_data`(
    p_action	VARCHAR(3),
	p_acctum	BIGINT
)
BEGIN
   ...
   DELETE FROM invdb.`acct_trade_preference`
    ...
    DELETE FROM invdb.`ext_nav_daily`
    where clientAccountID in (select distinct clientAccountID from cleanacct, invdb.ext_acct_info where cleanacct.acctnum = ext_acct_info.acctnum);
