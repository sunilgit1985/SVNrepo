...
BEGIN
    ...
    declare holidayDate   varchar(8);

    -- This is only to support older version...
	SET  currentDate = p_date;
    
		-- We are only going to reset the brokerdate if we have data on the system.
		select max(`ext_nav`.`reportdate`) into brokerdate from `invdb`.`ext_nav`;
		IF (brokerdate is NOT NULL)
		then
			 begin
    ...
    then
		-- get current business date AND intrement by 1.
		set currentDate=`invdb`.`FUNCT_GET_SWITCH`('BUSINESS_DATE');
	end if;
    
    -- Only move dates, if the current business date is set. [ This is safety, from other code ]
