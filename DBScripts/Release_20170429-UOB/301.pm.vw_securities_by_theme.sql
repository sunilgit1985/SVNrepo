...
VIEW `vw_securities_by_theme` AS
    SELECT DISTINCT
		...
		IFNULL(`user_basket_access`.`model`,
                'F') AS `model`,
        ...
        IFNULL(`sec_rbsa`.`ticker`,`sec_subasset_master`.`ticker`) AS `ticker`,
    ...
    FROM
		`user_basket_access`
        INNER JOIN `sec_subasset_master`
			ON (`user_basket_access`.`theme` = `sec_subasset_master`.`theme`)
		INNER JOIN `sec_master`
			ON (`sec_subasset_master`.`ticker` = `sec_master`.`ticker`
            AND  `sec_master`.`status` in ('A') )
		LEFT JOIN `sec_rbsa`
			ON (`sec_subasset_master`.`theme` = `sec_rbsa`.`theme`
			AND `sec_subasset_master`.`ticker` = `sec_rbsa`.`ticker`)
        LEFT JOIN `sec_asset_master`
			ON (`sec_subasset_master`.`theme` = `sec_asset_master`.`theme`
