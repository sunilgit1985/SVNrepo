drop procedure if exists rbsa.exchange_rate_processing;
delimiter $$
CREATE PROCEDURE rbsa.exchange_rate_processing(in p_ticker varchar(20),in p_dest_currency varchar(20))
 begin
   declare p_exchngsymbol,p_frstcd,p_scndcd,p_fromCurrency,p_toCurrency,baseCrncy varchar(10);
   declare p_exchngavailable int;
   declare p_multiplying_factor double;
 
   set p_exchngavailable=0;
   select currency_exchange_cd,multiplying_factor,base_currency into p_exchngsymbol,p_multiplying_factor,baseCrncy from invdb.sec_source_mapping where ticker_source_name=p_ticker and dest_currency=p_dest_currency;
   ...
   delete from rbsa.tmp_rbsa_daily where businessdate<(select min(exchangeDate) from rbsa.sec_exchangerate_data sed where sed.symbol=p_exchngsymbol );
   	 	 call rbsa.exchange_rate_processor(p_ticker,'forward',p_exchngsymbol,p_exchngsymbol,p_multiplying_factor,baseCrncy);
       else
   	...
   	select count(*) into p_exchngavailable from invdb.sec_exchangerate_master where symbol=p_exchngsymbol;
   		...
   		if(p_exchngavailable>0) then
              ...
              delete from rbsa.tmp_rbsa_daily where businessdate<(select min(exchangeDate) from rbsa.sec_exchangerate_data sed where sed.symbol=p_exchngsymbol );
   	  	 call rbsa.exchange_rate_processor(p_ticker,'backward',p_exchngsymbol,p_exchngsymbol,p_multiplying_factor,baseCrncy);
   		end if;
   end if;
