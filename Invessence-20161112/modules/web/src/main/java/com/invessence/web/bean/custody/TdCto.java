         ...
         case 9: // Funding Page 1
            if (tdMasterData.getFundNow() == false) // flag if opt of for funcindg is false
            {
               ...
               else if (tdMasterData.getFundType() != null && tdMasterData.getFundType().equalsIgnoreCase("PMACH"))
               {
                  ...
                  else if (tdMasterData.getInitialInvestment() < tdMasterData.getCustomerData().getInitialInvestment())
                  {
                     dataOK = false;
...
*/
   ...
   private String loadData()
   {
      ...
      // Although, we checked this on TOP (StartCTO), it is just so that if the method is called, we can stop.
      if (beanacctnum == null)
      {
      ...
      logonid = (webutil.isUserLoggedIn()) ? webutil.getLogonid() : getLongBeanlogonid();
...
/*    When use is not logged it, and account is not managed, then below check would allow anyone to view account.
      if (tdMasterData.getCustomerData().getLogonid().equals(logonid))
      {
...
*/
      ...
      if (tdMasterData.getCustomerData().getEditable())
      {
         ...
         // If user is not logged in, and if AcctType is filled in, then this someone has started the form.
         if (! webutil.isUserLoggedIn() && tdMasterData.getAcctdetail().getAcctTypeId() != null) {
            retValue = "differenUser";  // Don't know if the logonid, therefore, assume it is different User.
            return retValue;
         }
