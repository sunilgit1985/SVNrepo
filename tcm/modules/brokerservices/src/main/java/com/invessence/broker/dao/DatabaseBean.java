...
public class DatabaseBean {
    ...
    public void initialize() throws Exception {
        ...
        previousMonthDate = new SimpleJdbcCall(jdbc).withFunctionName("funct_get_switch").executeFunction(String.class,
                map);
        map.clear();
        ;

        map.put("vKey", "FIRST_DAY_OF_MONTH");
        firstDayOfMonth = new SimpleJdbcCall(jdbc).withFunctionName("funct_get_switch").executeFunction(String.class,
                map);
    }
    ...
    public void updateDate() {
        ...
        invokeSPToUploadRelAndUnrelFromTmpTable();

        updateBrokerDate();
    }

    private String getNextSummaryDateMonth() throws ParseException {
        return new SimpleDateFormat(YEAR_MONTH_FORMAT).format(DateUtils.addMonths(new SimpleDateFormat(
                YEAR_MONTH_FORMAT).parse(previousMonthDate), 1));
    }

    private String getNewMonthDate() throws ParseException {
        return new SimpleDateFormat(YEAR_MONTH_DAY_FORMAT).format(DateUtils.addMonths(new SimpleDateFormat(
                YEAR_MONTH_DAY_FORMAT).parse(firstDayOfMonth), 1));
    }
    ...
    private void postMsgtoDB(String subject, String msg, Boolean stopAndExit) {
        ...
        if (msg != null) {
            try {
                ...
                MapSqlParameterSource sqlParameterSource = new MapSqlParameterSource().addValue("p_addmodflag", "A")
                        .addValue("p_source", "Internal").addValue("p_messageid", null)
                        .addValue("p_sender", "no-replay@invessence.com")
                        .addValue("p_receiver", "supportl1@invessence.com").addValue("p_cc", null)
                        .addValue("p_bcc", null).addValue("p_subject", mySubject).addValue("p_status", 0)
                        .addValue("p_category", null).addValue("p_priority", null).addValue("p_logonid", null)
                        .addValue("p_sentdate", null).addValue("p_msg", msg).addValue("p_comment", "");
                new SimpleJdbcCall(dataSource).withProcedureName("sp_email_messages_add_mod").execute(
                        sqlParameterSource);
    ...
    private void invokeStoredProcToUploadAccountsFromTmpTable() {
        try {
            new SimpleJdbcCall(new JdbcTemplate(dataSource)).withProcedureName("sp_upload_IB_Accounts").execute(
                    Maps.newHashMap());
        } catch (Exception e) {
    ...
    private void invokeStoredProcToUploadPositionsFromTmpTable() {
        try {
            new SimpleJdbcCall(new JdbcTemplate(dataSource)).withProcedureName("sp_upload_position").execute(
                    Maps.newHashMap());
        } catch (Exception e) {
    ...
    private void invokeStoredProcToUploadCashReportsFromTmpTable() {
        try {
            new SimpleJdbcCall(new JdbcTemplate(dataSource)).withProcedureName("sp_upload_cash_report").execute(
                    Maps.newHashMap());
        } catch (Exception e) {
    ...
    private void invokeStoredProcToUploadCashTransactionFromTmpTable() {
        try {
            new SimpleJdbcCall(new JdbcTemplate(dataSource)).withProcedureName("sp_upload_cash_transaction").execute(
                    Maps.newHashMap());
        } catch (Exception e) {
    ...
    private void invokeStoredProcToUploadNavDailyFromTmpTable() {
        try {
            new SimpleJdbcCall(new JdbcTemplate(dataSource)).withProcedureName("sp_upload_nav_daily").execute(
                    Maps.newHashMap());
        } catch (Exception e) {
    ...
    private void invokeStoredProcToUploadCommissionFromTmpTable() {
        try {
            new SimpleJdbcCall(new JdbcTemplate(dataSource)).withProcedureName("sp_upload_commission").execute(
                    Maps.newHashMap());
        } catch (Exception e) {
    ...
    private void invokeStoredProcToUploadTradeFromTmpTable() {
        try {
            new SimpleJdbcCall(new JdbcTemplate(dataSource)).withProcedureName("sp_upload_trades").execute(
                    Maps.newHashMap());
        } catch (Exception e) {
    ...
    private void invokeSPToUploadUnbndledCommissionFromTmpTable() {
        try {
            new SimpleJdbcCall(new JdbcTemplate(dataSource)).withProcedureName("sp_upload_unbundld_commission")
    ...
    private void invokeSPToUploadRelAndUnrelFromTmpTable() {
        try {
            new SimpleJdbcCall(new JdbcTemplate(dataSource)).withProcedureName("sp_upload_relAndunrel").execute(
                    Maps.newHashMap());
        } catch (Exception e) {
            logger.error("Error invoking stored procedure to upload trade", e);
            postMsgtoDB("sp_upload_relAndunrel", "Unable to update Relized and Unrealized table", false);
        }
    }
