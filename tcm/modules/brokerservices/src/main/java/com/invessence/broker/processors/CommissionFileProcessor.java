...
public class CommissionFileProcessor
{
   private Logger logger = Logger.getLogger(CommissionFileProcessor.class.getName());
   ...
   public List<Map<String, String>> process()
   {
      ...
      try
      {
         clearCommissionTable();
         fileToProcess = FileProcessorUtil.getFileToProcess("Commission");
   ...
   private void clearCommissionTable()
   {
      JdbcTemplate clearTable = new JdbcTemplate(dataSource);

      String sql = "delete from tmp_commission";
      ...
      if (file != null)
      {
         ...
         try
         {
            ...
            for (String line : allLines)
            {
               ...
               for (int i = 0; i < BrokerServiceConstants.COMMISSION_TABLE_COLUMN.length; i++)
               {
                  map.put(BrokerServiceConstants.COMMISSION_TABLE_COLUMN[i], strings[i]);
               }
