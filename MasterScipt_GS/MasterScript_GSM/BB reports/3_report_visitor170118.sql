
/* report_visitor.sql170118_pmehta*/

DROP PROCEDURE IF EXISTS `report_visitors`;

DELIMITER $$
CREATE PROCEDURE `report_visitors`(
)
BEGIN

	SELECT user_trade_profile.acctnum
		   ,IFNULL(user_trade_profile.firstname, user_logon.firstname) as firstname
		   ,IFNULL(user_trade_profile.lastname, user_logon.lastname) as firstname
		   ,user_logon.email as email
           ,`visitor`.`ip`
           ,user_trade_profile.created as datetested
           ,user_trade_profile.goal as goal
           ,user_trade_profile.portfolioName as strategy
           ,user_trade_profile.initialInvestment as strategy
           ,CASE (user_trade_profile.status)
				WHEN 'V' THEN 'Visitor'
                WHEN 'N' THEN 'Pending'
                WHEN 'P' THEN 'Processing'
                ELSE 'Pending'
			END as `status`
		   ,IFNULL(`user_advisor_info`.`companyname`,`user_trade_profile`.`advisor`) as advisor
		   ,IFNULL(`user_advisor_info`.`displayname`,`user_trade_profile`.`rep`) as rep
           ,`user_risk_questions`.`age`
           ,`user_risk_questions`.`horizon`
           ,`user_risk_questions`.`ans3`
           ,`user_risk_questions`.`ans4`
           ,`user_risk_questions`.`ans5`
           , case WHEN (user_trade_profile.status = 'P') THEN 'TD'
				  WHEN (user_trade_profile.status = 'V')
						THEN
							CASE 
								WHEN (`user_risk_questions`.`ans5` > 0) THEN 'SignUp'
								WHEN (`user_risk_questions`.`ans4` > 0) THEN 'Page-4'
                                WHEN (`user_risk_questions`.`ans4` > 0) THEN 'Page-3'
                                ELSE 'Page-2'
							end
				  WHEN (user_trade_profile.status = 'N')
						THEN
							CASE 
								WHEN (`user_risk_questions`.`ans5` > 0) THEN 'App'
								WHEN (`user_risk_questions`.`ans4` > 0) THEN 'Page-4'
                                WHEN (`user_risk_questions`.`ans4` > 0) THEN 'Page-3'
                                ELSE 'Page-2'
							end
				  ELSE
					'NO Info'
			  end as info
 	FROM user_trade_profile
		INNER JOIN `user_risk_questions`
        ON (`user_trade_profile`.`acctnum` = `user_risk_questions`.`acctnum`)
		LEFT JOIN user_access_role
        ON (user_trade_profile.acctnum = user_access_role.acctnum)
		LEFT JOIN user_logon
        ON (user_access_role.logonid = user_logon.logonid
        AND `user_logon`.`logonid` > 1000)
        LEFT JOIN `user_advisor_info`
        ON ((`user_trade_profile`.`advisor` = `user_advisor_info`.`advisor`)
        AND CASE WHEN (`user_trade_profile`.`rep` = '') THEN 'CATCHALL'
				 ELSE (`user_trade_profile`.`rep`)
			END = `user_advisor_info`.`rep`)
        LEFT JOIN `visitor`
        ON (`user_trade_profile`.`acctnum` = `visitor`.`acctnum`)
	WHERE user_trade_profile.managed = 'N'
    AND IFNULL(user_logon.email,'XXX') not like '%invessence%'
    AND IFNULL(user_logon.email,'XXX') not like '%jvyas%'
    AND IFNULL(user_logon.email,'XXX') not like '%lengle%'
    AND IFNULL(user_logon.email,'XXX') not like '%pmehta%'
    AND IFNULL(user_logon.email,'XXX') not like '%prashant%'
    AND  user_trade_profile.created > DATE_ADD(now(),INTERVAL -7 DAY)
    ;

END$$
DELIMITER ;
