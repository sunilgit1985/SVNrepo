...
BEGIN
	...
	ELSE
		...
		and user_access_role.acctnum = profile.acctnum
		and user_access_role.role = 'OWNER'
		and profile.acctnum in (select DISTINCT uar.acctnum
								from user_access_role uar 
								WHERE IFNULL(p_logonid,uar.logonid) = uar.logonid)
		and IFNULL(p_filter,'ALL') = case WHEN (p_filter is null) then 'ALL'
										  WHEN (IB.accountStatus is null) THEN 'New'
										  ELSE IB.accountStatus
										end
		;
	END IF;
