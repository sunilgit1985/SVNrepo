...
public class CashTransactionFileProcessor
{
   private Logger logger = Logger.getLogger(CashTransactionFileProcessor.class.getName());
   ...
   public List<Map<String, String>> process()
   {
      ...
      try
      {
         ...
         fileToProcess = FileProcessorUtil.getFileToProcess("CashTransaction");
   ...
   private void clearCashTransactionTable()
   {
      ...
      String sql = "delete from tmp_cash_transaction";
      ...
      if (file != null)
      {
         ...
         try
         {
            ...
            for (String line : allLines)
            {
               ...
               for (int i = 0; i < BrokerServiceConstants.CASH_TRANSACTION_TABLE_COLUMN.length; i++)
               {
                  if ((strings.length-1) < i)
                     map.put(BrokerServiceConstants.CASH_TRANSACTION_TABLE_COLUMN[i], "");
                  else
                     map.put(BrokerServiceConstants.CASH_TRANSACTION_TABLE_COLUMN[i], strings[i]);
               }
