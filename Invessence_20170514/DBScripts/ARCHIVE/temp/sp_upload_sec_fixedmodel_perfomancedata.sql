...
DELIMITER $$
CREATE PROCEDURE `temp`.`sp_upload_sec_fixedmodel_perfomancedata`()
BEGIN
    ...
    FROM `tmp_sec_fixed_performancechart`
    GROUP BY `theme`, `level`;
    
    -- select * from tmp_theme_list;
    ...
    THEN
        ...
        WHERE `theme` in (SELECT `theme` from `tmp_theme_list`);
