...
{
   private Logger logger = Logger.getLogger(TradeFileProcessor.class.getName());
   ...
   public List<Map<String, String>> process()
   {
      ...
      {
         clearTradeTable();
         fileToProcess = FileProcessorUtil.getFileToProcess("Trade");
   ...
   private void clearTradeTable()
   {
      JdbcTemplate clearTable = new JdbcTemplate(dataSource);

      String sql = "delete from tmp_trades";
      ...
      if (file != null)
      {
         ...
         try
         {
            ...
            for (String line : allLines)
            {
               ...
               for (int i = 0; i < BrokerServiceConstants.TRADE_TABLE_COLUMN.length; i++)
               {
                  map.put(BrokerServiceConstants.TRADE_TABLE_COLUMN[i], strings[i]);
               }
