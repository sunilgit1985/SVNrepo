   ...
   public void doLogin() throws ServletException, IOException
   {
      ...
      if (e != null) {
         if (e instanceof LockedException) {
            uid = (UserInfoData) getCurrentInstance().getExternalContext().getSessionMap().get(Const.USER_INFO);
            if (uid != null) {
               String secureUrl = emailMessage.getMessagetext("secure.url", new Object[]{});

               String msg = "";
               System.out.println("LOGIN MIME TYPE: "+ uid.getEmailmsgtype());
               if(uid.getEmailmsgtype() != null && uid.getEmailmsgtype().equalsIgnoreCase("HTML")){
                  msg = emailMessage.getHTMLMessagetext("accountlocked.email.template", new Object[]{secureUrl, uid.getEmail(), uid.getResetID()});
               }else
               {
                  msg = emailMessage.getMessagetext("accountlocked.email", new Object[]{secureUrl, uid.getEmail(), uid.getResetID()});
               }
            ...
            String lockedMsg = emailMessage.getMessagetext("accountlocked.msg", new Object[]{});
            String redirectUrl="\\message.xhtml?faces-redirect=true&message=" + lockedMsg;
            uv.redirect(redirectUrl,null);
         }
         return;
      }
