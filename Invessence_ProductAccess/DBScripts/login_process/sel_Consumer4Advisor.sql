          ...
          THEN
				 ...
				 SELECT
					...
					WHERE IFNULL(`profile`.managed, 'N') = 'A'
						  ...
						  AND user.logonid = advisor_access.logonid
						  AND `profile`.acctnum = uar.acctnum
						  AND uar.role = 'OWNER'
						  AND IFNULL(`profile`.advisor, 'Invessence') LIKE tAdvisor
						  AND IFNULL(`profile`.rep, 'CATCHALL') LIKE tRep
		...
		WHEN (p_filter = 'N')
            ...
            FROM
              ...
              LEFT JOIN user_access_role uar
                ON (`profile`.acctnum = uar.acctnum
                    AND uar.role = 'OWNER')
              LEFT JOIN user_logon user
                ON (user.logonid = advisor_access.logonid)
          ...
          THEN
            ...
            WHERE IFNULL(`profile`.`status`, 'N') like '%' 
				  AND advisor_access.logonid = p_logonid
                  AND IFNULL(`profile`.advisor, 'Invessence') LIKE tAdvisor
                  AND IFNULL(`profile`.rep, 'CATCALL') LIKE tRep
			ORDER BY `profile`.`acctnum` desc;
        END CASE;
      END IF;
    END;
