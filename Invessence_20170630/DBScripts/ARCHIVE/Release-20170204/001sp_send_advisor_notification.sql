...
SELECT
    COUNT(*)
INTO tFound FROM
    notification_message_lookup
WHERE
    notification_message_lookup.messageType = UPPER(p_messagetype);
    ...
    IFNULL(ext_acct_info.applicantFName,
            user_trade_profile.firstname),
    IFNULL(ext_acct_info.applicantLName,
            user_trade_profile.lastname),
    user_trade_profile.portfolioName,
    user_trade_profile.goal,
    IFNULL(ext_acct_info.accountType,
            user_trade_profile.accttype),
    invdb.funct_get_actualCapital(user_trade_profile.acctnum),
    IFNULL(ext_acct_info.clientAccountID,
            user_trade_profile.acctnum)
INTO ttheme , tfirstname , tlastname , tportfolioName , tgoal , tacctType , tinvested , tclientAccountID FROM
    user_trade_profile
        LEFT JOIN
    ext_acct_info ON (user_trade_profile.acctnum = ext_acct_info.acctnum)
WHERE
    user_trade_profile.acctnum = p_acctnum
LIMIT 1;
