CREATE or REPLACE
 VIEW `vw_securities_by_theme` AS
    ...
    FROM
        `user_basket_access`
        JOIN `sec_assetclass_group` 
			ON (`user_basket_access`.`theme` = `sec_assetclass_group`.`theme`
            AND `sec_assetclass_group`.`status` = 'A')
        JOIN `sec_prime_asset_group` 
			ON (`sec_assetclass_group`.`theme` = `sec_prime_asset_group`.`theme`
            AND `sec_assetclass_group`.`assetclass` = `sec_prime_asset_group`.`assetclass`
            AND `sec_prime_asset_group`.`status` = 'A')
		LEFT JOIN `sec_rbsa`
			ON (`sec_rbsa`.`theme` = `sec_prime_asset_group`.`theme`
            AND `sec_rbsa`.`primeasset_ticker` = `sec_prime_asset_group`.`ticker`)
        LEFT JOIN `sec_master` 
			ON (`sec_master`.`ticker` = IFNULL(`sec_rbsa`.`ticker`,`sec_prime_asset_group`.`ticker`)
