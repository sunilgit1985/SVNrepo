...
public class CashReportFileProcessor
{
   private Logger logger = Logger.getLogger(CashReportFileProcessor.class.getName());
   ...
   private void clearCashTransactionTable()
   {
      ...
      String sql = "delete from tmp_cash_report";
      ...
      if (file != null)
      {
         ...
         try
         {
            ...
            for (String line : allLines)
            {
               ...
               for (int i = 0; i < BrokerServiceConstants.CASH_REPORT_TABLE_COLUMN.length; i++)
               {
                  if ((strings.length-1) < i)
                     map.put(BrokerServiceConstants.CASH_REPORT_TABLE_COLUMN[i], "");
                  else
                     map.put(BrokerServiceConstants.CASH_REPORT_TABLE_COLUMN[i], strings[i]);
               }
