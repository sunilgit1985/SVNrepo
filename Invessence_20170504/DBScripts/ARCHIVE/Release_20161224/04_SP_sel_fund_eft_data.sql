use invdb;
DROP PROCEDURE IF EXISTS `invdb`.`sel_fund_eft_data`;

DELIMITER $$
CREATE PROCEDURE `invdb`.`sel_fund_eft_data`(
...
WHERE
    b.tranStartDate IS not NULL and b.acctnum= `p_acctnum`
    and reqid=(select max(reqid) from dc_requests where acctnum=b.acctnum and dc_requestFor='EFT');
