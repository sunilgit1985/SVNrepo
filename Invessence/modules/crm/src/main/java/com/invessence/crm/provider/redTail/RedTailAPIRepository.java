...
import com.invessence.crm.dao.CRMAuditsDAO;
...
@Component
public class RedTailAPIRepository<T>
{
	@Autowired
	CRMAuditsDAO redTailAuditsDAO;
	private static final Logger logger = Logger.getLogger(RedTailAPIRepository.class);

	public AuthResponse authentication(ServiceRequest serviceRequest, User user)
	{
		System.out.println("serviceRequest = [" + serviceRequest + "], user = [" + user + "]");
		String baseUrl=null;
		...
		try
		{
			baseUrl = ServiceParameters.getConfigProperty(Constant.SERVICES.CRM_SERVICES.toString(), Constant.CRM_SERVICES.REDTAIL.toString(),"BASE_URL");
			authenticationUrl= ServiceParameters.getConfigProperty(Constant.SERVICES.CRM_SERVICES.toString(), Constant.CRM_SERVICES.REDTAIL.toString(),"AUTHENTICATION_URL");
			apiKey = ServiceParameters.getConfigProperty(Constant.SERVICES.CRM_SERVICES.toString(), Constant.CRM_SERVICES.REDTAIL.toString(),"API_KEY");

			String requestUrl=baseUrl+authenticationUrl;
			...
			getMethod.setRequestHeader("Content-Type","html/json");
			System.out.println("Response ************************************");
			String orig = apiKey+":"+user.getUserId()+":"+user.getPassword();

			//encoding  byte array into base 64
			byte[] encoded = Base64.encodeBase64(orig.getBytes());

			System.out.println("Original String: " + orig );
			System.out.println("Base64 Encoded String : Basic "+ new String(encoded));
		...
		{
			e.printStackTrace();
//			userProfile.setErrorStatus(new Status(CRMMessages.wsEGenErrCode, CRMMessages.wsEGenErrMsg));
		}
		logger.info("Out RedTailAPIRepository.registerUser()");
		return authResponse;
	}

	public AuthResponse ssoLogin(ServiceRequest serviceRequest, User user)
	{
		System.out.println("serviceRequest = [" + serviceRequest + "], user = [" + user + "]");
		String baseUrl=null;
		...
		{
			baseUrl = ServiceParameters.getConfigProperty(Constant.SERVICES.CRM_SERVICES.toString(), Constant.CRM_SERVICES.REDTAIL.toString(),"BASE_URL");
			ssoLoginUrl= ServiceParameters.getConfigProperty(Constant.SERVICES.CRM_SERVICES.toString(), Constant.CRM_SERVICES.REDTAIL.toString(),"SSO_URL");
			apiKey = ServiceParameters.getConfigProperty(Constant.SERVICES.CRM_SERVICES.toString(), Constant.CRM_SERVICES.REDTAIL.toString(),"API_KEY");

			String requestUrl=baseUrl+ssoLoginUrl;
			...
			getMethod.setRequestHeader("Content-Type","html/json");
			System.out.println("Response ************************************");
			String orig = apiKey+":"+user.getUserKey();

			//encoding  byte array into base 64
			byte[] encoded = Base64.encodeBase64(orig.getBytes());

			System.out.println("Original String: " + orig );
			System.out.println("Base64 Encoded String : Userkeyauth  "+ new String(encoded));
...
//			url = new ResponseParser<Url>().parse(Url.class, responseJSON);
//			System.out.println("url = " + url);
//			if(url==null){
//				error = new ResponseParser<Error>().parse(Error.class,responseJSON);
//				if(error==null){
//					result=new Url(null,null,null,new Status(CRMMessages.wsEGenErrCode, CRMMessages.wsEGenErrMsg));
//				}else{
