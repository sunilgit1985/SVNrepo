 ...
 */
@Service
public class CallingServiceFISExhange implements CallingExchangeService
{

   private static final Logger logger = Logger.getLogger(CallingServiceFISExhange.class);
...
//         ob.getDailyExchangeRate("http://91.212.43.32/XML/Quote.asp?SYMBOL=$$SYMBOL$$&Fields=name,last,bid,ask,high,low,date,open,close,time&username=vyas&usergroup=INVESSENCE&password=welcome", "IPXJ.L");
   ...
   public HistoricalData getDailyExchangeRate(String URL, String symbol,String exchangeDate) throws Exception
   {
      ...
      try
      {
         ...
         URL = URL.replace("$$SYMBOL$$", symbol)+ "&From=" + exchangeDate.replaceAll("-", "");
      ...
      try
      {
         ...
         while (bflag)
         {
            ...
            if (counter == 0)
            {
               ...
               strURL2 = strURL2.replace("$$DIRECTION$$", "Backward")+ "&To=" + exchangeDate.replaceAll("-", "");;
            }
            else
            {
               strURL2 = URL.replace("$$SYMBOL$$", symbol);
