...
public class FileProcessor
{
   ...
   public WSCallResult process(ServiceRequest serviceRequest, String lastProcessedDate){
      ...
      String existingBusinessDate= lastProcessedDate==null || lastProcessedDate.equals("")?null:lastProcessedDate;
      try
      {
         boolean flagCheckAddFiles;
         String processBasedOn=ServiceDetails.getConfigProperty(serviceRequest.getProduct(), Constant.SERVICES.FILE_PROCESS.toString(), serviceRequest.getMode(), "PROCESS_BASED_ON");

         dbParamMap = fileProcessorDao.getDBParametres();
         if (dbParamMap == null || dbParamMap.size() == 0 || !dbParamMap.containsKey("BUSINESS_DATE") || !dbParamMap.containsKey("SYSTEM_DATE") || !dbParamMap.containsKey("NEXT_BUSINESS_DATE"))
         {
         ...
         {
            businessDate=dbParamMap.get("BUSINESS_DATE").getValue().toString();
            processDate =  processBasedOn.equals("NEXT_BUSINESS_DATE")?dbParamMap.get("NEXT_BUSINESS_DATE").getValue().toString():processBasedOn.equals("SYSTEM_DATE")?dbParamMap.get("SYSTEM_DATE").getValue().toString():dbParamMap.get("BUSINESS_DATE").getValue().toString();

            logger.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
            logger.info(">>>>> File-Process Started for ProcessDate : "+processDate+"\n");

            if(existingBusinessDate !=null && businessDate.equals(existingBusinessDate))
            {
               mailAlertMsg.append("EOD Process did't change application dates for BusinessDate :"+existingBusinessDate+"\n");
            }else {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     Boolean isBussdtHoliday = fileProcessorDao.isBussdtHoliday(dbParamMap.get("SETT_CURRENCY").getValue().toString(),processDate);
                     int fileExecutionCounter = 1;

                     if (serviceRequest.getSequenceId() == 1)
                     {
                        flagExecuteParentProcess = true;
                     }
                        ...
                        while (itr.hasNext())
                        {
                           ...
                           if (fileDetails.getSeqNo() >= serviceRequest.getSequenceId())
                           {
                              ...
                              { // Checking Pre Instruction Process Result
                                 ...
                                 {
                                    fileProcessResult = fileDownloader.download(serviceRequest, fileDetails, fileRules, isProcessMailRaised, processDate);
                                 }
                                 else if (fileDetails.getProcess().equalsIgnoreCase("UPLOAD"))
                                 {
                                    ...
                                    {
                                       isProcessMailRaised = new StringBuilder();
                                       fileProcessResult = fileUploader.upload(serviceRequest, fileDetails, fileRules, isProcessMailRaised, processDate);
                                    }
                                 }
                              ...
                              FileDetails statusFile = new FileDetails(fileDetails.getVendor(), fileDetails.getFileName(), fileDetails.getProcessId(),
                                                                       ...
                                                                       fileProcessorUtil.getFilePath(serviceRequest, fileDetails, "LOCAL",processDate));
                     ...
                     { // Checking Process Execution Status
                        isProcessMailRaised = new StringBuilder();

                        if (isBussdtHoliday)
                        {
                           fileDetails = fileList.get(0);
                        }
   ...
   //                     }
      ...
      }finally
      {
         ...
         if (mailAlertMsg.length() > 0)
         {
            ...
            logger.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
            logger.info("Checking next day files for next day EOD Process");
            List<String> strList=fileProcessorUtil.getSFTPFileList(serviceRequest,fileDetails, processDate);
            if(fileProcessorUtil.isFilesAvailableForNextDay(strList,fileDetails,processDate)){
               process(serviceRequest, businessDate);
               existingBusinessDate=businessDate;
            }
