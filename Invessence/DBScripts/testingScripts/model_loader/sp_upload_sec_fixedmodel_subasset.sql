    ...
    ;
     ...
     BEGIN
        ...
        INSERT INTO `invdb`.`sec_subasset_master`
			(`theme`,`ticker`,`assetclass`,`subassetName`,`subassetcolor`,`sortorder`,`created`,`lastUpdated`)
		select distinct `tmp`.`theme`, `tmp`.`keyname`, `tmp`.`asset`, IFNULL(`tmp`.`keydescription`,`tmp`.`keyname`), 
        IFNULL(`tasset`.`color`,'#f5f5f5'), `tmp`.`sortorder`, now(), null
		from `temp`.`tmp_sec_fixedmodel_subasset` `tmp`,
			 `temp`.`tmp_sec_fixedmodel_asset` `tasset`
		where `tmp`.`theme` = `tasset`.`theme`
        AND   `tmp`.`asset` = `tasset`.`asset`
        AND   `tmp`.`level` = `tasset`.`level`
        AND   CONCAT(`tmp`.`theme`, `tmp`.`level`, `tmp`.`keyname`) in (
				SELECT DISTINCT CONCAT(`mastersub`.`theme`, `mastersub`.`level`, `mastersub`.`keyname`) 
				FROM `temp`.`tmp_sec_fixedmodel_subasset` as `mastersub`)
		order by 1,2
        ;
	END;
